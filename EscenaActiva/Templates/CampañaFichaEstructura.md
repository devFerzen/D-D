# CampañaFichaEstructura

## Identidad de campaña
- **Nombre de campaña:** 
- **Capítulo / arco actual:** 
- **Escena activa actual:** 
- **Pequeño contexto narrativo actual:** 

---

## Control macro narrativo de campaña
Este bloque mantiene foco en la historia principal sin perder subtramas útiles.

- **Arco principal activo:** 
  - **Estado actual:** 
  - **Próximo hito esperado:** 
  - **Referencia de soporte:** `Sesiones/sesionActual.md`

- **Subarcos activos:** 
  - **Subarco:** 
  - **Estado:** latente / activo / pausado / resuelto
  - **Dependencia con arco principal:** alta / media / baja
  - **Referencia de soporte:** `Sesiones/sesionActual.md` o `Personajes/<Nombre>/notas-campaña.md`

- **Balance canon / desvío (editable):**
  - **Nivel objetivo de desvío:** bajo / medio / alto
  - **Nivel real observado en sesión:** bajo / medio / alto
  - **Ajuste para próxima escena:** mantener / reconducir al canon / abrir desvío controlado

- **Regla global de retorno al foco principal:** 
  - **Disparador de reconducción:** 
  - **Máximo de escenas laterales antes de volver al foco:** 

---

## Estado global del grupo
- **Objetivo actual del grupo:** 
- **Momento del día:** mañana / tarde / noche / madrugada
- **Lugar general actual:** 
- **Ritmo o presión actual:** libre / moderado / urgente
- **Estado general de peligro:** bajo / medio / alto

---

## Personajes en escena
Resumen rápido. El detalle completo vive en `/Personajes`.

- **Nombre:** 
  - **Clase / nivel:** 
  - **HP actual / máximo:** 
  - **CA:** 
  - **Estado:** normal / herido / inconsciente / escondido / concentrando / etc.
  - **Posición en escena:** 
  - **Iniciativa actual:** 
  - **Salvaciones / ataque base relevante (simple):** 
  - **Notas rápidas:** 

- **Nombre:** 
  - **Clase / nivel:** 
  - **HP actual / máximo:** 
  - **CA:** 
  - **Estado:** 
  - **Posición en escena:** 
  - **Iniciativa actual:** 
  - **Salvaciones / ataque base relevante (simple):** 
  - **Notas rápidas:** 

---

## Orden de iniciativa
Usar solo si hay combate o secuencia por turnos.

1. 
2. 
3. 
4. 

---

## Conexiones y pistas entre escenas
Información relevante que enlaza escenas o ayuda al DM a recordar el progreso real.

- **Pista / conexión descubierta:** 
  - **Viene de la escena:** 
  - **Apunta hacia:** 
  - **Estado:** descubierta / parcial / no comprendida / pendiente

- **Pista / conexión descubierta:** 
  - **Viene de la escena:** 
  - **Apunta hacia:** 
  - **Estado:** 

---

## Resumen de avance narrativo de la sesión guardada
Síntesis para retomar campaña sin releer todo.

### Avances mayores de sesión
- **Pieza de historia:** 
  - **Qué avanzó:** 
  - **Impacto para próxima escena:** 
  - **Referencia de sesión:** `Sesiones/sesionActual.md` o archivo de sesión correspondiente

- **Pieza de historia:** 
  - **Qué avanzó:** 
  - **Impacto para próxima escena:** 
  - **Referencia de sesión:** 

### Avances menores o pendientes
- **Pieza de historia:** 
  - **Estado pendiente:** 
  - **Riesgo de olvido:** 
  - **Acción de reenganche sugerida:** 
  - **Referencia de sesión:** `Sesiones/sesionActual.md` o archivo de sesión correspondiente

- **Pieza de historia:** 
  - **Estado pendiente:** 
  - **Riesgo de olvido:** 
  - **Acción de reenganche sugerida:** 
  - **Referencia de sesión:** 

### Semillas narrativas para retomarlas después
- **Semilla:** 
  - **Estado:** sembrada / visible / activa / resuelta
  - **Cuándo retomarla:** próxima escena / próximo capítulo / oportunidad situacional
  - **Referencia de sesión:** `Sesiones/sesionActual.md` o archivo de sesión correspondiente

- **Semilla:** 
  - **Estado:** 
  - **Cuándo retomarla:** 
  - **Referencia de sesión:** 

---

## Dificultad dinámica para lo que este pasando en la escena activa / en desarrollo
No es solo “CD”, sino qué tan dura o fácil está la situación actual.

- **Nivel de dificultad actual:** 1 a 20
- **Razón narrativa o mecánica:** 
- **Qué facilita la situación ahora:** 
- **Qué complica la situación ahora:** 

### Si la dificultad baja
- más pistas
- menos presión
- menos enemigos
- más tiempo
- errores con menos castigo

### Si la dificultad sube
- más ruido
- más presión
- refuerzos
- menos visibilidad
- consecuencias más fuertes
- rutas bloqueadas

---

## Estado reactivo actual
- **Ruido global generado por el grupo:** bajo / medio / alto
- **Nivel de alerta enemiga:** bajo / medio / alto
- **Uso reciente de magia llamativa:** sí / no
- **Tiempo transcurrido desde evento importante:** 
- **Descanso posible o no:** 

---

## Notas globales para el DM
- **Eventos pendientes:** 
- **NPC relevantes fuera de escena:** 
- **Consecuencias si el grupo se demora:** 
- **Consecuencias si el grupo falla la escena actual:** 
- **Consecuencias si el grupo tiene éxito:** 